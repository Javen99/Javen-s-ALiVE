...
if (_name == "__SERVER__") then {
    ...
    if !(GVAR(DISABLED)) then {
        ...
        if (MOD(sys_data) getVariable ["saveCompositions","false"] == "true") then {
            ...
            if (((["ALiVE_civ_placement"] call ALiVE_fnc_isModuleAvailable) || {(["ALiVE_civ_placement_custom"] call ALiVE_fnc_isModuleAvailable)}) && {!isnil "ALIVE_CIV_PLACEMENT_ROADBLOCK_LOCATIONS"}) then {
